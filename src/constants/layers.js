// ─────────────────────────────────────────────────────────────
// Layer group definitions used by both the sidebar and the map.
//
// Each layer has a unique `key` that acts as the source-of-truth
// identifier between the LayerPanel checkboxes and the map rendering.
// ─────────────────────────────────────────────────────────────
...
/**
 * Build the initial layer visibility state from LAYER_GROUPS defaults.
 * Returns: { city: true, node1: false, … }
 */
export function buildDefaultLayers() {
  return LAYER_GROUPS.flatMap(g => g.layers).reduce(
    (acc, layer) => ({ ...acc, [layer.key]: layer.defaultOn }),
    {},
  );
}
